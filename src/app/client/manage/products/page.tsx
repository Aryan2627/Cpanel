...
export default function ProductsPage() {
  ...
  const kpis = [
    { label:'Total Products', value:products.length, icon:Package, color:'#0f172a', bg:'#f8faff' },
    ...
    <div style={{ backgroundColor:'#f0f4f8',minHeight:'100%',fontFamily:'system-ui,sans-serif' }}>
      <div style={{ background: '#fff', borderBottom: '1px solid #e5edff',padding:'28px 32px 40px',position:'relative',overflow:'hidden' }}>
        ...
        <div style={{ position:'relative',zIndex:1,display:'flex',justifyContent:'space-between',alignItems:'flex-start' }}>
          <div>
            <div style={{ display:'flex',alignItems:'center',gap:'10px',marginBottom:'8px' }}><Package size={18} color="rgba(255,255,255,0.55)" /><p style={{ color: '#475569',fontSize:'0.72rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.08em',margin:0 }}>Master Data</p></div>
            <h1 style={{ color: '#0f172a', fontSize: '1.8rem',fontWeight:800,margin:'0 0 6px',letterSpacing:'-0.5px' }}>Products</h1>
            <p style={{ color: '#475569',margin:0,fontSize:'0.9rem' }}>Manage your product catalog and item master.</p>
          </div>
          <div style={{ display:'flex',gap:'10px' }}>
            ...
            <Link href="/client/manage/products/create" style={{ display:'flex',alignItems:'center',gap:'7px',padding:'10px 20px',background:'#0f172a',border:'none',borderRadius:'10px',color:'#fff',fontWeight:700,fontSize:'0.875rem',textDecoration:'none',boxShadow:'0 4px 14px rgba(37,99,235,0.35)' }}><Plus size={17} /> Add Product</Link>
          </div>
        </div>
      </div>
        ...
        <div style={{ backgroundColor:'#fff',borderRadius:'16px',border:'1px solid #e2e8f0',boxShadow: 'none',overflow:'hidden' }}>
          <div style={{ padding:'0 20px',borderBottom:'1px solid #f1f5f9',display:'flex',alignItems:'center',gap:'4px',background:'#fafbfc',flexWrap:'wrap' }}>
            {categories.slice(0,6).map(c=>(
              <button key={c} onClick={()=>setFilterCategory(c)} style={{ padding:'14px 12px',border:'none',borderBottom:filterCategory===c?'2px solid #0f172a':'2px solid transparent',background:'transparent',cursor:'pointer',fontSize:'0.78rem',fontWeight:filterCategory===c?700:500,color:filterCategory===c?'#0f172a':'#475569',transition:'all 0.15s',whiteSpace:'nowrap' }}>{c}</button>
            ))}
            <div style={{ flex:1 }} />
          ...
          <table style={{ width:'100%',borderCollapse:'collapse' }}>
            ...
            <tbody>
              {filtered.length>0 ? filtered.map((p,idx)=>{
                ...
                return (
                  <tr key={p.id} style={{ borderBottom:'1px solid #f1f5f9',background:isSel?'#f8faff':idx%2===0?'#fff':'#fafbfc',borderLeft:isSel?'3px solid #0f172a':'3px solid transparent',transition:'all 0.12s' }}
                    ...
                    <td style={{ padding:'13px 16px' }}><input type="checkbox" checked={isSel} onChange={()=>{const s=new Set(selectedIds);isSel?s.delete(p.id):s.add(p.id);setSelectedIds(s);}} style={{ accentColor:'#0f172a',cursor:'pointer' }} /></td>
                    <td style={{ padding:'13px 16px' }}><span style={{ background:'#f8faff',color:'#1e3a8a',padding:'3px 8px',borderRadius:'5px',fontFamily:'monospace',fontSize:'0.72rem',fontWeight:700 }}>{p.code||'—'}</span></td>
                    <td style={{ padding:'13px 16px',fontWeight:700,color:'#0f172a',fontSize:'0.875rem' }}>{p.name}</td>
                    ...
                    <td style={{ padding:'13px 16px',color:'#475569',fontSize:'0.82rem' }}>{p.uom||'EA'}</td>
                    <td style={{ padding:'13px 16px',fontWeight:600,color:'#0f172a' }}>{p.stock??'—'}</td>
                    <td style={{ padding:'13px 16px' }}>
                      <div style={{ display:'flex',gap:'6px' }}>
                        <Link href={`/client/manage/products/edit/${p.id}`} style={{ display:'flex',alignItems:'center',gap:'4px',padding:'5px 10px',background:'#f8faff',border:'1px solid #e5edff',borderRadius:'7px',color:'#0f172a',fontWeight:700,fontSize:'0.72rem',textDecoration:'none' }}><Eye size={12} /> View</Link>
                      </div>
                    </td>
                  </tr>
                );
          ...
          {filtered.length>0&&(
            <div style={{ padding:'12px 20px',borderTop:'1px solid #f1f5f9',display:'flex',justifyContent:'space-between',alignItems:'center',background:'#fafbfc' }}>
              <div style={{ fontSize:'0.8rem',color:'#475569' }}>Showing <strong style={{ color:'#0f172a' }}>{filtered.length}</strong> of <strong style={{ color:'#0f172a' }}>{products.length}</strong> products{selectedIds.size>0&&<span style={{ marginLeft:'12px',color:'#0f172a',fontWeight:600 }}>· {selectedIds.size} selected</span>}</div>
            </div>
          )}
        </div>
      </div>
        ...
        <div style={{ position:'fixed',inset:0,background:'rgba(15,23,42,0.55)',backdropFilter:'blur(4px)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center' }} onClick={()=>setShowBulkModal(false)}>
          <div style={{ background:'#fff',borderRadius:'20px',width:'440px',maxWidth:'92vw',boxShadow:'0 30px 60px rgba(0,0,0,0.2)',overflow:'hidden' }} onClick={e=>e.stopPropagation()}>
            <div style={{ background: '#fff', borderBottom: '1px solid #e5edff',padding:'22px 24px',display:'flex',justifyContent:'space-between',alignItems:'center' }}>
              ...
              <button onClick={()=>setShowBulkModal(false)} style={{ background:'rgba(255,255,255,0.1)',border:'none',cursor:'pointer',color: '#0f172a', width: '32px',height:'32px',borderRadius:'8px',display:'flex',alignItems:'center',justifyContent:'center' }}><X size={16} /></button>
            </div>
            <div style={{ padding:'24px',display:'flex',flexDirection:'column',gap:'12px' }}>
              <p style={{ color:'#475569',fontSize:'0.875rem',margin:0,lineHeight:1.6 }}>Upload an Excel file with columns: Code, Name, Category, Unit Price, UOM, Stock.</p>
              <div style={{ border:'2px dashed #e2e8f0',borderRadius:'12px',padding:'32px',textAlign:'center',background:'#f8fafc' }}>
                ...
                <div style={{ fontWeight:600,color:'#475569',fontSize:'0.875rem' }}>Drag & drop or click to upload</div>
                <div style={{ color:'#94a3b8',fontSize:'0.75rem',marginTop:'4px' }}>.xlsx, .csv accepted</div>
              </div>
              <button onClick={()=>setShowBulkModal(false)} style={{ padding:'11px',background:'#0f172a',color:'#fff',border:'none',borderRadius:'10px',fontWeight:700,fontSize:'0.875rem',cursor:'pointer' }}>Upload File</button>
            </div>
          </div>
        </div>
      )}
