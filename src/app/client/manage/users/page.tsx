...
export default function UsersPage() {
  ...
  const kpis = [
    { label: 'Total Users', value: users.length, icon: Users, color: '#0f172a', bg: '#f8faff' },
  ...
  const roleStyle = (r: string): React.CSSProperties => {
    ...
    if (r==='Manager') return { background:'#f8faff',color:'#0f172a',border:'1px solid #e5edff' };
        ...
        <div style={{ position:'relative',zIndex:1,display:'flex',justifyContent:'space-between',alignItems:'flex-start' }}>
          <div>
            <div style={{ display:'flex',alignItems:'center',gap:'10px',marginBottom:'8px' }}>
              ...
              <p style={{ color: '#475569',fontSize:'0.72rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.08em',margin:0 }}>Master Data</p>
            </div>
            <h1 style={{ color: '#0f172a', fontSize: '1.8rem',fontWeight:800,margin:'0 0 6px',letterSpacing:'-0.5px' }}>Users</h1>
            <p style={{ color: '#475569',margin:0,fontSize:'0.9rem' }}>Manage portal users, roles, and permissions.</p>
          </div>
          <button onClick={() => { setIsEditMode(false); setFormData({name:'',email:'',phone:'',role:'Buyer',erpId:'',status:'Active',department:''}); setIsCreateModalOpen(true); }}
            style={{ display:'flex',alignItems:'center',gap:'7px',padding:'10px 20px',background:'#0f172a',border:'none',borderRadius:'10px',color:'#fff',fontWeight:700,fontSize:'0.875rem',cursor:'pointer',boxShadow:'0 4px 14px rgba(37,99,235,0.35)' }}>
      ...
      <div style={{ padding:'0 32px 40px', marginTop:'-24px', position:'relative', zIndex:10 }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'16px',marginBottom:'20px' }}>
          {kpis.map((k,i) => { const Icon=k.icon; return (
            <div key={i} style={{ backgroundColor:'#fff',borderRadius:'16px',border:'1px solid #e2e8f0',padding:'20px 22px',boxShadow: 'none',display:'flex',justifyContent:'space-between',alignItems:'flex-start',transition:'all 0.2s',cursor:'pointer' }}
              ...
              <div><div style={{ fontSize:'0.68rem',fontWeight:700,color:'#475569',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:'8px' }}>{k.label}</div>
        ...
        <div style={{ backgroundColor:'#fff',borderRadius:'16px',border:'1px solid #e2e8f0',boxShadow: 'none',overflow:'hidden' }}>
          ...
          <div style={{ padding:'0 20px',borderBottom:'1px solid #f1f5f9',display:'flex',alignItems:'center',gap:'4px',background:'#fafbfc',flexWrap:'wrap' }}>
            {roles.map(r => (
              <button key={r} onClick={()=>setFilterRole(r)} style={{ padding:'14px 14px',border:'none',borderBottom:filterRole===r?'2px solid #0f172a':'2px solid transparent',background:'transparent',cursor:'pointer',fontSize:'0.8rem',fontWeight:filterRole===r?700:500,color:filterRole===r?'#0f172a':'#475569',transition:'all 0.15s',whiteSpace:'nowrap' }}>
          ...
          <table style={{ width:'100%',borderCollapse:'collapse' }}>
            <thead>
              <tr style={{ background: '#f8faff', borderBottom: '1px solid #e5edff' }}>
                <th style={{ padding:'13px 16px',width:'44px' }}>
                  <input type="checkbox" checked={selectedIds.size===filtered.length&&filtered.length>0} onChange={e=>setSelectedIds(e.target.checked?new Set(filtered.map(u=>u.id)):new Set())} style={{ accentColor:'#0f172a',cursor:'pointer' }} />
                </th>
            ...
            <tbody>
              {filtered.length>0 ? filtered.map((u,idx)=>{
                ...
                return (
                  <tr key={u.id} style={{ borderBottom:'1px solid #f1f5f9',background:isSel?'#f8faff':idx%2===0?'#fff':'#fafbfc',borderLeft:isSel?'3px solid #0f172a':'3px solid transparent',transition:'all 0.12s' }}
                    ...
                    <td style={{ padding:'13px 16px' }}><input type="checkbox" checked={isSel} onChange={()=>{const s=new Set(selectedIds);isSel?s.delete(u.id):s.add(u.id);setSelectedIds(s);}} style={{ accentColor:'#0f172a',cursor:'pointer' }} /></td>
                    <td style={{ padding:'13px 16px' }}>
                      <div style={{ display:'flex',alignItems:'center',gap:'10px' }}>
                        <div style={{ width:'32px',height:'32px',borderRadius:'50%',background: '#f8faff',color: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '0.75rem',flexShrink:0 }}>
                ...
                <tr><td colSpan={8} style={{ padding:'64px',textAlign:'center' }}>
                  <div style={{ display:'flex',flexDirection:'column',alignItems:'center',gap:'12px' }}>
                    ...
                    <div style={{ color:'#475569',fontSize:'0.875rem' }}>Add your first user to get started.</div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  {['Purchase Orders', 'Purchase Requests', 'Sourcing Events', 'Vendors Data'].map((module) => (
                     <label key={module} style={{ display:'flex',alignItems:'center',gap:'8px',fontSize:'0.8rem',color:'#475569',cursor:'pointer' }}>
                        <input type="checkbox" defaultChecked={true} style={{ accentColor: '#0f172a', width: '16px', height: '16px' }} /> {module}
                     </label>
                  ))}
                </div>
              </div>
