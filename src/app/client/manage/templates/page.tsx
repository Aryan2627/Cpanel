...
export default function TemplatesPage() {
  ...
  const kpis = [
    { label:'Total Templates', value:templates.length, icon:LayoutTemplate, color:'#0f172a', bg:'#f8faff' },
    ...
    <div style={{ backgroundColor:'#f0f4f8',minHeight:'100%',fontFamily:'system-ui,sans-serif' }}>
      <div style={{ background: '#fff', borderBottom: '1px solid #e5edff',padding:'28px 32px 40px',position:'relative',overflow:'hidden' }}>
        ...
        <div style={{ position:'relative',zIndex:1,display:'flex',justifyContent:'space-between',alignItems:'flex-start' }}>
          <div>
            <div style={{ display:'flex',alignItems:'center',gap:'10px',marginBottom:'8px' }}><FileCode2 size={18} color="rgba(255,255,255,0.55)" /><p style={{ color: '#475569',fontSize:'0.72rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.08em',margin:0 }}>Master Data</p></div>
            <h1 style={{ color: '#0f172a', fontSize: '1.8rem',fontWeight:800,margin:'0 0 6px',letterSpacing:'-0.5px' }}>Event Templates</h1>
            <p style={{ color: '#475569',margin:0,fontSize:'0.9rem' }}>Build and manage RFQ, Technical, and Auction templates.</p>
          </div>
          <Link href="/client/manage/templates/create" style={{ display:'flex',alignItems:'center',gap:'7px',padding:'10px 20px',background:'#0f172a',border:'none',borderRadius:'10px',color:'#fff',fontWeight:700,fontSize:'0.875rem',textDecoration:'none',boxShadow:'0 4px 14px rgba(37,99,235,0.35)' }}><Plus size={17} /> New Template</Link>
        </div>
      </div>
      ...
      <div style={{ padding:'0 32px 40px',marginTop:'-24px',position:'relative',zIndex:10 }}>
        ...
        <div style={{ backgroundColor:'#fff',borderRadius:'16px',border:'1px solid #e2e8f0',boxShadow: 'none',overflow:'hidden' }}>
          <div style={{ padding:'0 20px',borderBottom:'1px solid #f1f5f9',display:'flex',alignItems:'center',gap:'4px',background:'#fafbfc' }}>
            {types.map(t=>(
              <button key={t} onClick={()=>setFilterType(t)} style={{ padding:'14px 14px',border:'none',borderBottom:filterType===t?'2px solid #0f172a':'2px solid transparent',background:'transparent',cursor:'pointer',fontSize:'0.8rem',fontWeight:filterType===t?700:500,color:filterType===t?'#0f172a':'#475569',transition:'all 0.15s',display:'flex',alignItems:'center',gap:'6px' }}>
          ...
          filtered.length>0 ? (
              ...
              {filtered.map(t => {
                ...
                return (
                  <div key={t.id} style={{ borderRadius:'14px',border:'1px solid #e2e8f0',background:'#fff',boxShadow:'0 2px 8px rgba(0,0,0,0.04)',overflow:'hidden',transition:'all 0.2s' }}
                    ...
                    <div style={{ background: '#f8faff', borderBottom: '1px solid #e5edff',padding:'16px 18px',display:'flex',justifyContent:'space-between',alignItems:'flex-start' }}>
                    ...
                    <div style={{ padding:'16px 18px' }}>
                      ...
                      <p style={{ margin:'0 0 14px',color:'#475569',fontSize:'0.8rem',lineHeight:1.5 }}>{t.description||'No description provided.'}</p>
                      ...
                      <div style={{ display:'flex',gap:'8px' }}>
                        <Link href={`/client/manage/templates/${t.id}`} style={{ flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:'5px',padding:'8px',background:'#f8faff',border:'1px solid #e5edff',borderRadius:'8px',color:'#0f172a',fontWeight:700,fontSize:'0.78rem',textDecoration:'none' }}><Play size={13} /> Use</Link>
            ...
            <div style={{ padding:'64px',textAlign:'center' }}>
              <div style={{ display:'flex',flexDirection:'column',alignItems:'center',gap:'12px' }}>
                ...
                <div style={{ color:'#475569',fontSize:'0.875rem' }}>Create your first template to get started.</div>
          ...
          {filtered.length>0&&<div style={{ padding:'12px 20px',borderTop:'1px solid #f1f5f9',background:'#fafbfc',fontSize:'0.8rem',color:'#475569' }}><strong style={{ color:'#0f172a' }}>{filtered.length}</strong> of <strong style={{ color:'#0f172a' }}>{templates.length}</strong> templates</div>}
        </div>
      </div>
    </div>
  );
