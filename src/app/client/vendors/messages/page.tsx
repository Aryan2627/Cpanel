    ...
    <div style={{backgroundColor:'#f0f4f8',minHeight:'100%',fontFamily:'system-ui,sans-serif'}}>
      ...
      <div style={{background: '#fff', borderBottom: '1px solid #e5edff',padding:'28px 32px 40px',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{position:'relative',zIndex:1}}>
          <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'8px'}}><MessageSquare size={18} color="rgba(255,255,255,0.55)"/><p style={{color: '#475569',fontSize:'0.72rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.08em',margin:0}}>Vendors</p></div>
          <h1 style={{color: '#0f172a', fontSize: '1.8rem',fontWeight:800,margin:'0 0 6px',letterSpacing:'-0.5px'}}>Secure Messaging</h1>
          <p style={{color: '#475569',margin:0,fontSize:'0.9rem'}}>Communicate privately with invited suppliers for an active event.</p>
        </div>
      </div>

      <div style={{padding:'0 32px 40px',marginTop:'-24px',position:'relative',zIndex:10}}>
        <div style={{backgroundColor:'#fff',borderRadius:'16px',border:'1px solid #e2e8f0',boxShadow: 'none',overflow:'hidden',display:'flex',height:'calc(100vh - 220px)',minHeight:'560px'}}>
          ...
          <div style={{width:'300px',borderRight:'1px solid #f1f5f9',display:'flex',flexDirection:'column',flexShrink:0}}>
            <div style={{padding:'16px',borderBottom:'1px solid #f1f5f9',background:'#fafbfc'}}>
              <div style={{fontSize:'0.68rem',fontWeight:700,color:'#475569',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:'10px'}}>Event ID</div>
            ...
            <div style={{padding:'10px 16px',borderBottom:'1px solid #f1f5f9',background:'#fafbfc'}}>
              <div style={{fontSize:'0.68rem',fontWeight:700,color:'#475569',textTransform:'uppercase',letterSpacing:'0.05em'}}>Invited Suppliers</div>
            </div>
            <div style={{flex:1,overflowY:'auto'}}>
              {vendors.map(v=>(
                <div key={v.id} onClick={()=>setActiveVendor(v.name)}
                  style={{padding:'14px 16px',borderBottom:'1px solid #f8fafc',cursor:'pointer',borderLeft:activeVendor===v.name?'3px solid #0f172a':'3px solid transparent',background:activeVendor===v.name?'#f8faff':'transparent',transition:'all 0.15s'}}
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                    <div style={{width:'36px',height:'36px',borderRadius:'10px',background:activeVendor===v.name?'linear-gradient(135deg,#0d1f4f,#0f172a)':'linear-gradient(135deg,#94a3b8,#475569)',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:800,fontSize:'0.8rem',flexShrink:0,transition:'all 0.15s'}}>
                      {v.name.charAt(0).toUpperCase()}
                    </div>
                    <div style={{flex:1,overflow:'hidden'}}>
                      <div style={{fontWeight:700,color:activeVendor===v.name?'#0f172a':'#0f172a',fontSize:'0.85rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{v.name}</div>
          ...
          {activeEvent ? (
            <>
                ...
                {activeVendor ? (
                  <>
                    ...
                    <div style={{padding:'14px 20px',borderBottom:'1px solid #f1f5f9',display:'flex',alignItems:'center',gap:'12px',background:'#fff',boxShadow:'0 1px 4px rgba(0,0,0,0.04)'}}>
                      <div style={{width:'38px',height:'38px',borderRadius:'10px',background: '#f8faff',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:800,fontSize:'0.85rem'}}>{activeVendor.charAt(0).toUpperCase()}</div>
                      <div>
                        <div style={{fontWeight:800,color:'#0f172a',fontSize:'0.95rem'}}>{activeVendor}</div>
                        <div style={{display:'flex',alignItems:'center',gap:'5px'}}><div style={{width:'7px',height:'7px',borderRadius:'50%',background:'#16a34a'}}/><span style={{fontSize:'0.72rem',color:'#475569'}}>Active Now</span></div>
                      </div>
                    </div>
                    {/* Messages */}
                    <div style={{flex:1,padding:'20px',overflowY:'auto',display:'flex',flexDirection:'column',gap:'16px',background:'#f8fafc',backgroundImage:'radial-gradient(#e2e8f0 1px,transparent 1px)',backgroundSize:'24px 24px'}}>
                      {messages.map(msg=>(
                        <div key={msg.id} style={{display:'flex',flexDirection:'column',alignItems:msg.sender==='me'?'flex-end':'flex-start'}}>
                          <div style={{maxWidth:'72%',padding:'10px 14px',borderRadius:msg.sender==='me'?'16px 16px 2px 16px':'16px 16px 16px 2px',background:msg.sender==='me'?'linear-gradient(135deg,#0f172a,#0f172a)':'#fff',color:msg.sender==='me'?'#fff':'#0f172a',boxShadow:'0 2px 8px rgba(0,0,0,0.08)',fontSize:'0.875rem',lineHeight:1.5,display:'flex',alignItems:'center',gap:'7px',border:msg.sender==='me'?'none':'1px solid #e2e8f0'}}>
                            {msg.isFile&&<FileText size={16} color={msg.sender==='me'?'rgba(255,255,255,0.7)':'#0f172a'}/>}{msg.text}
                          </div>
                    ...
                    <div style={{padding:'14px 20px',borderTop:'1px solid #f1f5f9',background:'#fff'}}>
                      <div style={{display:'flex',alignItems:'center',gap:'8px',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:'12px',padding:'6px 6px 6px 14px',transition:'all 0.15s'}}
                        onFocus={e=>(e.currentTarget as HTMLElement).style.borderColor='#0f172a'} onBlur={e=>(e.currentTarget as HTMLElement).style.borderColor='#e2e8f0'}>
                        ...
                        <button onClick={()=>handleSend()} disabled={!newMessage.trim()}
                          style={{width:'36px',height:'36px',borderRadius:'9px',border:'none',background:newMessage.trim()?'#0f172a':'#e2e8f0',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',cursor:newMessage.trim()?'pointer':'not-allowed',flexShrink:0,transition:'all 0.15s'}}>
                          <Send size={15}/>
                        </button>
                      </div>
                    </div>
                  ...
                  <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:'12px',background:'#f8fafc'}}>
                    ...
                    <div style={{color:'#475569',fontSize:'0.82rem'}}>Choose a vendor from the list to start messaging.</div>
                  </div>
                )}
              </div>
              ...
              <div style={{width:'300px',borderLeft:'1px solid #f1f5f9',display:'flex',flexDirection:'column',flexShrink:0}}>
                <div style={{padding:'16px',borderBottom:'1px solid #f1f5f9',background: '#f8faff', borderBottom: '1px solid #e5edff'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'4px'}}><FileText size={15} color="rgba(255,255,255,0.6)"/><span style={{fontSize:'0.68rem',fontWeight:700,color: '#475569',textTransform:'uppercase',letterSpacing:'0.05em'}}>Event</span></div>
                  <div style={{fontWeight:800,color:'#fff',fontSize:'0.95rem',marginBottom:'3px'}}>{activeEvent.title||activeEvent.refId}</div>
                  <div style={{fontSize:'0.72rem',color: '#475569'}}>{activeEvent.refId}</div>
                </div>
                <div style={{flex:1,padding:'16px',overflowY:'auto'}}>
                  <div style={{fontSize:'0.72rem',fontWeight:700,color:'#475569',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:'12px',display:'flex',alignItems:'center',gap:'6px'}}><IndianRupee size={13}/>Quotation</div>
                  {vendorBid ? (
                    <div style={{background:'#fff',borderRadius:'12px',border:'1px solid #e2e8f0',padding:'16px',boxShadow:'0 2px 8px rgba(0,0,0,0.04)'}}>
                      ...
                      <div style={{marginBottom:'16px'}}>
                        <div style={{fontSize:'0.75rem',color:'#475569',marginBottom:'4px'}}>Total Bid</div>
                        <div style={{fontSize:'1.5rem',fontWeight:800,color:'#0f172a'}}>{vendorBid.currency} {vendorBid.amount?.toLocaleString()}</div>
                      </div>
                      {vendorBid.templateData&&(()=>{try{const items=typeof vendorBid.templateData==='string'?JSON.parse(vendorBid.templateData):vendorBid.templateData;if(Array.isArray(items))return(<div style={{borderTop:'1px solid #f1f5f9',paddingTop:'12px'}}><div style={{fontSize:'0.72rem',fontWeight:700,color:'#475569',marginBottom:'8px'}}>Item Breakdown</div>{items.map((it:any,i:number)=><div key={i} style={{display:'flex',justifyContent:'space-between',marginBottom:'6px',fontSize:'0.8rem'}}><span style={{color:'#475569'}}>{it.itemName||it.name||`Item ${i+1}`}</span><span style={{fontWeight:600,color:'#0f172a'}}>{it.price||it.total||'—'}</span></div>)}</div>);}catch{return null;}})()||null}
                    </div>
                  ) : (
                    <div style={{background:'#f8fafc',borderRadius:'12px',border:'1px solid #e2e8f0',padding:'24px',textAlign:'center'}}>
                      ...
                      <div style={{fontWeight:600,color:'#475569',fontSize:'0.82rem'}}>No bid submitted yet</div>
            ...
            <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:'16px',background:'#f8fafc'}}>
              ...
              <div style={{color:'#475569',fontSize:'0.875rem',maxWidth:'340px',textAlign:'center',lineHeight:1.6}}>Enter an Event ID on the left to securely message invited suppliers and review their quotations.</div>
            </div>
          )}
        </div>
      </div>
