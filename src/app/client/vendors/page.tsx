...
export default function VendorManagement() {
  ...
  const statusStyle=(s:string):React.CSSProperties=>{
    ...
    if(s==='Invited') return {background:'#f8faff',color:'#0f172a',border:'1px solid #e5edff'};
  ...
  const kpis=[
    {label:'Total Vendors',value:vendors.length,icon:Building2,color:'#0f172a',bg:'#f8faff'},
    ...
    <div style={{backgroundColor:'#f0f4f8',minHeight:'100%',fontFamily:'system-ui,sans-serif'}}>
      ...
      <div style={{background: '#fff', borderBottom: '1px solid #e5edff',padding:'28px 32px 40px',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{position:'relative',zIndex:1,display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
          <div>
            <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'8px'}}><Building2 size={18} color="rgba(255,255,255,0.55)"/><p style={{color: '#475569',fontSize:'0.72rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.08em',margin:0}}>Vendors</p></div>
            <h1 style={{color: '#0f172a', fontSize: '1.8rem',fontWeight:800,margin:'0 0 6px',letterSpacing:'-0.5px'}}>Supplier List</h1>
            <p style={{color: '#475569',margin:0,fontSize:'0.9rem'}}>Manage your vendor network, onboard suppliers and track status.</p>
          </div>
          <button onClick={()=>setIsInviteOpen(true)} style={{display:'flex',alignItems:'center',gap:'7px',padding:'10px 20px',background:'#0f172a',border:'none',borderRadius:'10px',color:'#fff',fontWeight:700,fontSize:'0.875rem',cursor:'pointer',boxShadow:'0 4px 14px rgba(37,99,235,0.35)'}}>
      ...
      <div style={{padding:'0 32px 40px',marginTop:'-24px',position:'relative',zIndex:10}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'16px',marginBottom:'20px'}}>
          {kpis.map((k,i)=>{const Icon=k.icon;return(
            <div key={i} style={{backgroundColor:'#fff',borderRadius:'16px',border:'1px solid #e2e8f0',padding:'20px 22px',boxShadow: 'none',display:'flex',justifyContent:'space-between',alignItems:'flex-start',transition:'all 0.2s',cursor:'pointer'}}
              ...
              <div><div style={{fontSize:'0.68rem',fontWeight:700,color:'#475569',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:'8px'}}>{k.label}</div><div style={{fontSize:'2rem',fontWeight:800,color:'#0f172a',letterSpacing:'-0.05em',lineHeight:1}}>{k.value}</div></div>
        ...
        {/* Table Card */}
        <div style={{backgroundColor:'#fff',borderRadius:'16px',border:'1px solid #e2e8f0',boxShadow: 'none',overflow:'hidden'}}>
          {/* Tabs + Search */}
          <div style={{padding:'0 20px',borderBottom:'1px solid #f1f5f9',display:'flex',alignItems:'center',gap:'2px',background:'#fafbfc',flexWrap:'wrap'}}>
            {statusTabs.map(tab=>(
              <button key={tab} onClick={()=>setFilterStatus(tab)} style={{padding:'14px 12px',border:'none',borderBottom:filterStatus===tab?'2px solid #0f172a':'2px solid transparent',background:'transparent',cursor:'pointer',fontSize:'0.78rem',fontWeight:filterStatus===tab?700:500,color:filterStatus===tab?'#0f172a':'#475569',transition:'all 0.15s',whiteSpace:'nowrap',display:'flex',alignItems:'center',gap:'6px'}}>
                ...
                <tr key={v.id} style={{borderBottom:'1px solid #f1f5f9',background:idx%2===0?'#fff':'#fafbfc',borderLeft:'3px solid transparent',transition:'all 0.12s'}}
                  ...
                  <td style={{padding:'13px 16px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                      <div style={{width:'34px',height:'34px',borderRadius:'10px',background: '#f8faff',color: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '0.75rem',flexShrink:0}}>{(v.name||'V').charAt(0).toUpperCase()}</div>
                  ...
                  <td style={{padding:'13px 16px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'3px'}}>
                      ...
                      <span style={{marginLeft:'4px',fontSize:'0.72rem',color:'#475569',fontWeight:600}}>4.0</span>
                    </div>
                  </td>
                  <td style={{padding:'13px 16px'}}>
                    <div style={{display:'flex',gap:'6px',flexWrap:'nowrap'}}>
                      <button onClick={()=>router.push(`/client/vendors/${v.id}`)} style={{display:'flex',alignItems:'center',gap:'4px',padding:'5px 10px',background:'#f8faff',border:'1px solid #e5edff',borderRadius:'7px',color:'#0f172a',fontWeight:700,fontSize:'0.72rem',cursor:'pointer'}}><Eye size={12}/> View</button>
          ...
          {filtered.length>0&&(
            <div style={{padding:'12px 20px',borderTop:'1px solid #f1f5f9',display:'flex',justifyContent:'space-between',alignItems:'center',background:'#fafbfc'}}>
              <div style={{fontSize:'0.8rem',color:'#475569'}}><strong style={{color:'#0f172a'}}>{filtered.length}</strong> of <strong style={{color:'#0f172a'}}>{vendors.length}</strong> vendors</div>
            </div>
          )}
        </div>
      </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.55)',backdropFilter:'blur(4px)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setIsInviteOpen(false)}>
          <div style={{background:'#fff',borderRadius:'20px',width:'480px',maxWidth:'92vw',boxShadow:'0 30px 60px rgba(0,0,0,0.2)',overflow:'hidden'}} onClick={e=>e.stopPropagation()}>
            <div style={{background: '#fff', borderBottom: '1px solid #e5edff',padding:'22px 24px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              ...
              <button onClick={()=>setIsInviteOpen(false)} style={{background:'rgba(255,255,255,0.1)',border:'none',cursor:'pointer',color: '#0f172a', width: '32px',height:'32px',borderRadius:'8px',display:'flex',alignItems:'center',justifyContent:'center'}}><X size={16}/></button>
            </div>
            <div style={{padding:'24px',display:'flex',flexDirection:'column',gap:'14px'}}>
              {[['name','Company Name'],['email','Email Address'],['vendorCode','Vendor Code'],['city','City']].map(([f,l])=>(
                <div key={f}><label style={{display:'block',fontSize:'0.75rem',fontWeight:700,color:'#475569',marginBottom:'5px',textTransform:'uppercase',letterSpacing:'0.04em'}}>{l}</label>
              ...
              <div><label style={{display:'block',fontSize:'0.75rem',fontWeight:700,color:'#475569',marginBottom:'5px',textTransform:'uppercase',letterSpacing:'0.04em'}}>Vendor Type</label>
              ...
              <div style={{display:'flex',gap:'10px',marginTop:'4px'}}>
                ...
                <button onClick={handleSubmit} disabled={isSubmitting} style={{flex:2,padding:'11px',background:'#0f172a',color:'#fff',border:'none',borderRadius:'10px',fontWeight:700,cursor:'pointer'}}>{isSubmitting?'Sending...':'Send Invitation'}</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.55)',backdropFilter:'blur(4px)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setSelectedVendorForApproval(null)}>
          <div style={{background:'#fff',borderRadius:'20px',width:'480px',maxWidth:'92vw',boxShadow:'0 30px 60px rgba(0,0,0,0.2)',overflow:'hidden'}} onClick={e=>e.stopPropagation()}>
            <div style={{background: '#fff', borderBottom: '1px solid #e5edff',padding:'22px 24px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              ...
              <button onClick={()=>setSelectedVendorForApproval(null)} style={{background:'rgba(255,255,255,0.1)',border:'none',cursor:'pointer',color: '#0f172a', width: '32px',height:'32px',borderRadius:'8px',display:'flex',alignItems:'center',justifyContent:'center'}}><X size={16}/></button>
            </div>
            <div style={{padding:'24px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'14px',padding:'16px',background:'#f8fafc',borderRadius:'12px',marginBottom:'20px'}}>
                <div style={{width:'48px',height:'48px',borderRadius:'12px',background: '#f8faff',color: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '1.1rem'}}>{selectedVendorForApproval.name?.charAt(0)}</div>
                <div><div style={{fontWeight:800,color:'#0f172a',fontSize:'1rem'}}>{selectedVendorForApproval.name}</div>
                <div style={{color:'#475569',fontSize:'0.82rem'}}>{selectedVendorForApproval.email}</div>
