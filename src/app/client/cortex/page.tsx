                          ...
                          <div className="cx-form-grid-2" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                            <div>
                              ...
                              <div style={{position:'relative'}}>
                                ...
                                <input type="number" min="0" step="0.01" onWheel={(e)=>(e.target as any).blur()} placeholder="0.00" value={s2pForm.budget} onChange={e=>setS2pForm({...s2pForm, budget:e.target.value})} style={{ width:'100%', padding:'10px 13px 10px 24px', borderRadius:'10px', border:'1px solid rgba(255,255,255,0.08)', background:'rgba(255,255,255,0.04)', color:'#f1f5f9', fontSize:'0.84rem', outline:'none', boxSizing:'border-box' }} onFocus={e=>{e.target.style.border='1px solid rgba(99,102,241,0.5)';e.target.style.boxShadow='0 0 0 3px rgba(99,102,241,0.1)';}} onBlur={e=>{e.target.style.border='1px solid rgba(255,255,255,0.08)';e.target.style.boxShadow='none';}}/>
                              </div>
                            </div>
                            <div>
                        ...
                        <div style={{ padding:'18px 20px', display:'flex', flexDirection:'column', gap:'13px' }}>
                          ...
                          <div>
                            ...
                            <div style={{position:'relative'}}>
                              ...
                              <input type="number" min="0" step="0.01" onWheel={(e)=>(e.target as any).blur()} placeholder="0.00" value={eventForm.budget} onChange={e=>setEventForm({...eventForm, budget:e.target.value})} style={{ width:'100%', padding:'10px 13px 10px 24px', borderRadius:'6px', border: isDark ? '1px solid #334155' : '1px solid #cbd5e1', background: isDark ? '#0f172a' : '#ffffff', color: isDark ? '#f8fafc' : '#0f172a', fontSize:'0.875rem', outline:'none', boxSizing:'border-box', transition:'all 0.2s' }} onFocus={e=>{e.target.style.border='1px solid #2563eb';e.target.style.boxShadow='0 0 0 2px rgba(37,99,235,0.2)';}} onBlur={e=>{e.target.style.border=isDark ? '1px solid #334155' : '1px solid #cbd5e1';e.target.style.boxShadow='none';}}/>
                            </div>
                          </div>
                          <div>
                            ...
                            <div className="cx-form-grid-2" style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:'8px', alignItems:'center' }}>
                              <input type="number" min="1" step="1" onWheel={(e)=>(e.target as any).blur()} placeholder="e.g. 7" value={eventForm.duration} onChange={e=>setEventForm({...eventForm, duration:e.target.value})} style={{ padding:'10px 13px', borderRadius:'6px', border: isDark ? '1px solid #334155' : '1px solid #cbd5e1', background: isDark ? '#0f172a' : '#ffffff', color: isDark ? '#f8fafc' : '#0f172a', fontSize:'0.875rem', outline:'none', boxSizing:'border-box', transition:'all 0.2s' }} onFocus={e=>{e.target.style.border='1px solid #2563eb';e.target.style.boxShadow='0 0 0 2px rgba(37,99,235,0.2)';}} onBlur={e=>{e.target.style.border=isDark ? '1px solid #334155' : '1px solid #cbd5e1';e.target.style.boxShadow='none';}}/>
                    ...
                    <div style={{ marginTop:'14px', padding:'18px', borderRadius:'14px', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)' }}>
                      ...
                      <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                        {[{ p:'Description / Purpose', k:'desc', t:'text' },{ p:'PO Number (Optional)', k:'poNumber', t:'text' },{ p:'Total Amount ($)', k:'amount', t:'number', min:'0', step:'0.01' }].map(f=>(
                          <input key={f.k} type={f.t} min={f.min} step={f.step} onWheel={f.t==='number'? (e)=>(e.target as any).blur() : undefined} placeholder={f.p} value={(poForm as any)[f.k]} onChange={e=>setPoForm({...poForm, [f.k]:e.target.value})} style={{ width:'100%', padding:'10px 14px', borderRadius:'8px', border:'1px solid rgba(255,255,255,0.1)', background:'rgba(255,255,255,0.05)', color:'#e2e8f0', fontSize:'0.85rem', outline:'none' }}/>
                        ))}
