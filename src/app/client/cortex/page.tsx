              ...
              <div key={idx} className="cortex-msg cx-msg-bubble" style={{ display:'flex', gap:'14px', alignItems:'flex-start' }}>
                ...
                <div style={{ flex:1, paddingTop:'4px', minWidth:0 }}>
                  ...
                  {msg.uiComponent==='event_creation_form' && (
                      <div className="cx-form-card cx-event-card" style={{ marginTop:'16px', borderRadius:'14px', overflow:'hidden', border:'1px solid rgba(0,198,255,0.15)', background: isDark ? 'linear-gradient(145deg,rgba(15,20,30,0.97),rgba(10,15,20,0.98))' : '#ffffff', boxShadow: isDark ? '0 8px 30px rgba(0,0,0,0.3),inset 0 1px 0 rgba(255,255,255,0.02)' : '0 4px 20px rgba(0,198,255,0.05)' }}>
                        <div style={{ padding:'16px 20px 14px', borderBottom:'1px solid rgba(255,255,255,0.05)', background:'rgba(0,198,255,0.03)', display:'flex', alignItems:'center', gap:'10px' }}>
                          <div style={{ width:'30px', height:'30px', borderRadius:'8px', background:'linear-gradient(135deg,#00c6ff,#0072ff)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.9rem', boxShadow:'0 4px 10px rgba(0,198,255,0.2)', flexShrink:0 }}>✨</div>
                          <div>
                            <div style={{ color:'#f1f5f9', fontWeight:600, fontSize:'0.88rem' }}>Create Sourcing Event</div>
                            <div style={{ color:'rgba(148,163,184,0.6)', fontSize:'0.7rem', marginTop:'1px' }}>Configure and publish a live procurement event</div>
                          </div>
                        </div>
                        <div style={{ padding:'18px 20px', display:'flex', flexDirection:'column', gap:'13px' }}>
                          <div>
                            <label style={{ display:'block', color:'rgba(148,163,184,0.65)', fontSize:'0.68rem', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:'6px' }}>Event Title <span style={{color:'#38bdf8'}}>*</span></label>
                            <input type="text" placeholder="e.g. Q4 Cloud Infrastructure RFQ" value={eventForm.title} onChange={e=>setEventForm({...eventForm, title:e.target.value})} style={{ width:'100%', padding:'10px 13px', borderRadius:'8px', border:'1px solid rgba(255,255,255,0.08)', background:'rgba(255,255,255,0.02)', color:'#f1f5f9', fontSize:'0.84rem', outline:'none', boxSizing:'border-box', transition:'all 0.2s' }} onFocus={e=>{e.target.style.border='1px solid rgba(0,198,255,0.4)';e.target.style.boxShadow='0 0 0 3px rgba(0,198,255,0.05)';}} onBlur={e=>{e.target.style.border='1px solid rgba(255,255,255,0.08)';e.target.style.boxShadow='none';}}/>
                          </div>
                          <div>
                            ...
                            <div style={{position:'relative'}}>
                              <span style={{position:'absolute',left:'11px',top:'50%',transform:'translateY(-50%)',color:'rgba(148,163,184,0.35)',fontSize:'0.85rem',fontWeight:700,pointerEvents:'none'}}>$</span>
                              <input type="text" placeholder="0.00" value={eventForm.budget} onChange={e=>setEventForm({...eventForm, budget:e.target.value})} style={{ width:'100%', padding:'10px 13px 10px 24px', borderRadius:'8px', border:'1px solid rgba(255,255,255,0.08)', background:'rgba(255,255,255,0.02)', color:'#f1f5f9', fontSize:'0.84rem', outline:'none', boxSizing:'border-box', transition:'all 0.2s' }} onFocus={e=>{e.target.style.border='1px solid rgba(0,198,255,0.4)';e.target.style.boxShadow='0 0 0 3px rgba(0,198,255,0.05)';}} onBlur={e=>{e.target.style.border='1px solid rgba(255,255,255,0.08)';e.target.style.boxShadow='none';}}/>
                            </div>
                          </div>
                          <div>
                            <label style={{ display:'block', color:'rgba(148,163,184,0.65)', fontSize:'0.68rem', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:'6px' }}>Duration <span style={{color:'#38bdf8'}}>*</span></label>
                            <div className="cx-form-grid-2" style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:'8px', alignItems:'center' }}>
                              <input type="number" onWheel={(e)=>(e.target as any).blur()} placeholder="e.g. 7" value={eventForm.duration} onChange={e=>setEventForm({...eventForm, duration:e.target.value})} style={{ padding:'10px 13px', borderRadius:'8px', border:'1px solid rgba(255,255,255,0.08)', background:'rgba(255,255,255,0.02)', color:'#f1f5f9', fontSize:'0.84rem', outline:'none', boxSizing:'border-box', transition:'all 0.2s' }} onFocus={e=>{e.target.style.border='1px solid rgba(0,198,255,0.4)';e.target.style.boxShadow='0 0 0 3px rgba(0,198,255,0.05)';}} onBlur={e=>{e.target.style.border='1px solid rgba(255,255,255,0.08)';e.target.style.boxShadow='none';}}/>
                              <div style={{ display:'flex', gap:'6px' }}>
                                {(['minutes','days'] as const).map(unit => (
                                  <button key={unit} onClick={()=>setEventForm({...eventForm, durationUnit:unit})} style={{ padding:'10px 14px', borderRadius:'8px', border:'1px solid', borderColor: eventForm.durationUnit===unit ? 'rgba(0,198,255,0.4)' : 'rgba(255,255,255,0.08)', background: eventForm.durationUnit===unit ? 'rgba(0,198,255,0.1)' : 'rgba(255,255,255,0.02)', color: eventForm.durationUnit===unit ? '#00c6ff' : 'rgba(148,163,184,0.55)', fontSize:'0.78rem', fontWeight:600, cursor:'pointer', textTransform:'capitalize', fontFamily:'inherit', transition:'all 0.15s', whiteSpace:'nowrap' }}>
                          ...
                          <div style={{ paddingTop:'8px', borderTop:'1px solid rgba(255,255,255,0.05)' }}>
                            <button onClick={()=>execute('/execute-create-event '+JSON.stringify(eventForm))} style={{ width:'100%', padding:'12px', background:'linear-gradient(135deg,#00c6ff,#0072ff)', color:'#fff', border:'none', borderRadius:'8px', fontWeight:600, cursor:'pointer', fontSize:'0.87rem', letterSpacing:'0.02em', boxShadow:'0 4px 12px rgba(0,198,255,0.2)', display:'flex', alignItems:'center', justifyContent:'center', gap:'7px', fontFamily:'inherit' }}>
                              ✨ Publish Event Now
                            </button>
                          </div>
                        </div>
