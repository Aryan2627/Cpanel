...
export default function PurchaseOrdersPage() {
  ...
  const kpis = [
    { label: 'Total POs', value: pos.length, icon: FileText, color: '#0f172a', bg: '#f8faff' },
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e5edff', padding: '28px 32px 40px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <h1 style={{ color: '#0f172a', fontSize: '1.8rem', fontWeight: 800, margin: '0 0 6px', letterSpacing: '-0.5px' }}>Purchase Orders</h1>
            <p style={{ color: '#475569', margin: 0, fontSize: '0.9rem' }}>Track and manage financial commitments to vendors.</p>
          </div>
      ...
      <div style={{ padding: '0 32px 40px', marginTop: '-24px', position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ backgroundColor: '#fff', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: 'none', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '0 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '4px', background: '#fafbfc', flexWrap: 'wrap' }}>
            {statusTabs.map(tab => (
              <button key={tab} onClick={() => setFilterStatus(tab)} style={{ padding: '14px 14px', border: 'none', borderBottom: filterStatus === tab ? '2px solid #0f172a' : '2px solid transparent', background: 'transparent', cursor: 'pointer', fontSize: '0.8rem', fontWeight: filterStatus === tab ? 700 : 500, color: filterStatus === tab ? '#0f172a' : '#475569', transition: 'all 0.15s', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '6px' }}>
                {tab}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.length > 0 ? filtered.map((po, idx) => {
                ...
                return (
                  <tr key={po.id} style={{ borderBottom: '1px solid #f1f5f9', background: isSelected ? '#f8faff' : idx % 2 === 0 ? '#fff' : '#fafbfc', borderLeft: isSelected ? '3px solid #0f172a' : '3px solid transparent', transition: 'all 0.12s' }}
                    ...
                    <td style={{ padding: '13px 16px' }}>
                      <input type="checkbox" checked={isSelected} onChange={() => { const s = new Set(selectedIds); isSelected ? s.delete(po.id) : s.add(po.id); setSelectedIds(s); }} style={{ accentColor: '#0f172a', cursor: 'pointer' }} />
                    </td>
                    <td style={{ padding: '13px 16px' }}>
                      <span style={{ background: '#f8faff', color: '#1e3a8a', padding: '3px 8px', borderRadius: '5px', fontFamily: 'monospace', fontSize: '0.72rem', fontWeight: 700 }}>{po.poNumber}</span>
                    </td>
                    <td style={{ padding: '13px 16px', fontWeight: 700, color: '#0f172a', fontSize: '0.875rem', maxWidth: '220px' }}>
                      <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{po.title}</div>
                    </td>
                    <td style={{ padding: '13px 16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: '#f8faff', color: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '0.65rem', flexShrink: 0 }}>
                    ...
                    <td style={{ padding: '13px 16px', color: '#475569', fontSize: '0.82rem' }}>
                    ...
                    <td style={{ padding: '13px 16px' }}>
                      <Link href={`/client/po/${po.id}`}
                        style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '6px 12px', background: '#f8faff', border: '1px solid #e5edff', borderRadius: '7px', color: '#0f172a', fontWeight: 700, fontSize: '0.75rem', textDecoration: 'none', whiteSpace: 'nowrap', transition: 'all 0.15s' }}
                ...
                <tr>
                  <td colSpan={8} style={{ padding: '64px', textAlign: 'center' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div style={{ color: '#475569', fontSize: '0.875rem' }}>Try adjusting your search or status filter.</div>
                    </div>
                  </td>
                </tr>
              )}
