  ...
  return (
    <div style={{ backgroundColor: '#f8fafc', color: '#333', minHeight: '100%', padding: '24px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px', marginBottom: '32px' }}>
        {[
          { label: 'Total Requests', value: totalRequests, icon: Inbox },
          { label: 'Pending Approval', value: pendingRequests, icon: Clock },
          { label: 'Approved', value: approvedRequests, icon: CheckCircle2 },
        ].map((stat, i) => (
          <div key={i} style={{ backgroundColor: '#fff', padding: '24px', borderRadius: '8px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <span style={{ color: '#475569', fontSize: '0.875rem', fontWeight: 500 }}>{stat.label}</span>
              <stat.icon size={18} color="#94a3b8" />
            </div>
            <h3 style={{ margin: 0, color: '#0f172a', fontSize: '2rem', fontWeight: 600, letterSpacing: '-1px' }}>{stat.value}</h3>
          </div>
        ))}
      </div>
      ...
      {/* Slide-out Drawer for Quick View - SIMPLE UI */}
      {selectedIntake && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.4)', zIndex: 100, display: 'flex', justifyContent: 'flex-end' }} onClick={() => setSelectedIntake(null)}>
          <div style={{ width: '450px', backgroundColor: '#ffffff', height: '100%', boxShadow: '-5px 0 15px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ padding: '24px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <span style={{ fontSize: '0.8rem', color: '#64748b' }}>{selectedIntake.refId}</span>
                <h2 style={{ fontSize: '1.25rem', margin: '4px 0 8px 0', color: '#0f172a' }}>{selectedIntake.title}</h2>
                <p style={{ margin: 0, color: '#475569', fontSize: '0.9rem' }}>Requested by {selectedIntake.reqName}</p>
              </div>
            ...
            <div style={{ padding: '24px', flex: 1, overflowY: 'auto' }}>
              <div style={{ marginBottom: '24px' }}>
                <h3 style={{ fontSize: '0.9rem', color: '#64748b', marginBottom: '8px' }}>Status</h3>
                <div>{getStatusBadge(selectedIntake.status)}</div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px', marginBottom: '32px' }}>
                <div style={{ padding: '12px', border: '1px solid #e2e8f0', borderRadius: '6px' }}>
                  <span style={{ fontSize: '0.8rem', color: '#64748b' }}>Category / Type</span>
                  <div style={{ fontSize: '0.95rem', color: '#0f172a', marginTop: '4px' }}>{selectedIntake.type}</div>
                </div>
                <div style={{ padding: '12px', border: '1px solid #e2e8f0', borderRadius: '6px' }}>
                  <span style={{ fontSize: '0.8rem', color: '#64748b' }}>Assigned Buyer</span>
                  <div style={{ fontSize: '0.95rem', color: '#0f172a', marginTop: '4px' }}>{selectedIntake.buyer}</div>
                </div>
                <div style={{ padding: '12px', border: '1px solid #e2e8f0', borderRadius: '6px' }}>
                  <span style={{ fontSize: '0.8rem', color: '#64748b' }}>Requested Date</span>
                  <div style={{ fontSize: '0.95rem', color: '#0f172a', marginTop: '4px' }}>{selectedIntake.reqAt}</div>
                </div>
                <div style={{ padding: '12px', border: '1px solid #e2e8f0', borderRadius: '6px' }}>
                  <span style={{ fontSize: '0.8rem', color: '#64748b' }}>Total Quantity</span>
                  <div style={{ fontSize: '0.95rem', color: '#0f172a', marginTop: '4px' }}>{selectedIntake.quantity || 1}</div>
                </div>
              </div>

              <div style={{ padding: '16px', border: '1px solid #e2e8f0', borderRadius: '6px' }}>
                <h3 style={{ fontSize: '0.9rem', color: '#0f172a', margin: '0 0 12px 0' }}>Activity</h3>
                <div style={{ fontSize: '0.9rem', color: '#475569' }}>
                  <p style={{ margin: '0 0 4px 0' }}><strong>Created:</strong> {selectedIntake.reqAt}</p>
                  <p style={{ margin: 0 }}><strong>Status:</strong> Waiting for manager review</p>
                </div>
              </div>
            </div>
