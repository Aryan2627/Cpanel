const fs = require('fs');
let code = fs.readFileSync('src/app/client/events/create/single-stage/page.tsx', 'utf8');

// Add state for Modal
code = code.replace(/const \[isVendorDropdownOpen, setIsVendorDropdownOpen\] = useState\(false\);/, 
...
code = code.replace(/\{\/\* Card 3: Participants \*\/\}[\s\S]*?No vendors added yet\.\n\s*<\/div>\n\s*\)\}\n\s*<\/div>\n\s*<\/div>/, newParticipantsCard);

// Add the Vendor Selection Modal UI at the bottom before final closing tags
const modalUI = `
...
fs.writeFileSync('src/app/client/events/create/single-stage/page.tsx', code, 'utf8');
console.log('Fixed Single Stage Modal');
