...
const regex = /\/\/ Semantic AI Intelligence & Market Refinement[\s\S]*?if \(prTitle\.includes\('laptop'\)[\s\S]*?\}\s*const target = prevAmount \+ \(prevAmount \* \(trend \/ 100\)\);/m;
