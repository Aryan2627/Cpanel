...
// The Compare Matrix button uses BarChart3
const replaceRegex = /<button onClick=\{\(\) => setIsCompareModalOpen\(true\)\}[\s\S]*?Compare Matrix\s*<\/button>/g;

const splitBtn = `<button onClick={() => setIsCompareModalOpen(true)} style={{ backgroundColor: '#10b981', color: '#fff', padding: '6px 16px', border: 'none', borderRadius: '6px', fontSize: '0.85rem', fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 1px 2px rgba(16, 185, 129, 0.2)' }}>
...
code = code.replace(replaceRegex, splitBtn);
...
code = code.replace('{isCompareModalOpen && (', splitAwardModal + '\n      {isCompareModalOpen && (');
