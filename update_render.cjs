...
function updateRenderBlock(file) {
    let code = fs.readFileSync(file, 'utf8');

    const searchStr = "return (\n                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>\n                        {uniqueCreatorFields.map((f: any) => (";
    
    // Find where the map ends. We know the structure.
    // It ends with:
    //                         ))}
    //                       </div>
    //                     );
...
updateRenderBlock('src/app/client/events/create/single-stage/page.tsx');
updateRenderBlock('src/app/client/events/create/auction/page.tsx');
